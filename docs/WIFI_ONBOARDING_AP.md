...
This guide documents the Wi-Fi onboarding flow for firmware builds that include the ESPAgent onboarding portal.

## What It Does

When onboarding is enabled, ESPAgent can expose a local Wi-Fi access point such as `ESPAgent-XXXX` and serve a configuration page at `http://192.168.4.1`.
...
- a phone or laptop that can join the temporary `ESPAgent-XXXX` network

## First-Time Setup

1. Power on the device.
2. Wait for the onboarding hotspot `ESPAgent-XXXX` to appear.
...
If the firmware keeps the admin AP online after normal Wi-Fi connection, you can reconnect to `ESPAgent-XXXX` later and open `http://192.168.4.1` again.
...
1. saved NVS values
2. build-time defaults from `main/espagent_secrets.h`

That means:

- if a field exists in NVS, it overrides the build-time default
- if you clear a saved field and reboot, the device falls back to `main/espagent_secrets.h`
...
```text
ESPAgent> config_reset
```

## Troubleshooting

### No `ESPAgent-XXXX` hotspot appears
...
If the device already has `model` or `provider` saved in NVS, the saved values win over `main/espagent_secrets.h`.
