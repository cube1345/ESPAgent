...
esp_err_t tool_cron_add_execute(const char *input_json, char *output, size_t output_size)
{
    ...
    if (strcmp(schedule_type, "every") == 0) {
        ...
        if (!hour || !cJSON_IsNumber(hour) || hour->valuedouble < 0 ||
            hour->valuedouble > 23 || !minute || !cJSON_IsNumber(minute) ||
            minute->valuedouble < 0 || minute->valuedouble > 59) {
            snprintf(output, output_size,
                     "Error: 'daily' schedule requires hour 0-23 and minute 0-59");
            cJSON_Delete(root);
            return ESP_ERR_INVALID_ARG;
        }
        job.hour = (uint8_t)hour->valuedouble;
        job.minute = (uint8_t)minute->valuedouble;
        job.delete_after_run = false;
    } else {
        snprintf(output, output_size, "Error: schedule_type must be 'every', 'at', or 'daily'");
        cJSON_Delete(root);
        return ESP_ERR_INVALID_ARG;
    }
    ...
    if (job.kind == CRON_KIND_EVERY) {
        ...
        snprintf(output, output_size,
                 "OK: Added daily job '%s' (id=%s), runs at %02u:%02u local time. Next run at epoch %lld.",
                 job.name, job.id, (unsigned)job.hour, (unsigned)job.minute,
...
esp_err_t tool_cron_list_execute(const char *input_json, char *output, size_t output_size)
{
    ...
    for (int i = 0; i < count && off < output_size - 1; i++) {
        ...
        if (j->kind == CRON_KIND_EVERY) {
            ...
            off += snprintf(output + off, output_size - off,
                "  %d. [%s] \"%s\" — daily at %02u:%02u, %s, next=%lld, last=%lld, ch=%s:%s\n",
                i + 1, j->id, j->name,
                (unsigned)j->hour, (unsigned)j->minute,
                j->enabled ? "enabled" : "disabled",
                (long long)j->next_run, (long long)j->last_run,
                j->channel, j->chat_id);
        } else {
