...
static int64_t cron_next_daily_epoch(uint8_t hour, uint8_t minute, time_t now)
{
    struct tm local_tm;
...
static esp_err_t cron_load_jobs(void)
{
    ...
    cJSON_ArrayForEach(item, jobs_arr) {
        ...
        if (strcmp(kind_str, "every") == 0) {
            ...
            int h = (hour && cJSON_IsNumber(hour)) ? (int)hour->valuedouble : -1;
            int m = (minute && cJSON_IsNumber(minute)) ? (int)minute->valuedouble : -1;
...
static void cron_process_due_jobs(void)
{
    ...
    for (int i = 0; i < s_job_count; i++) {
        ...
        if (job->kind == CRON_KIND_AT) {
            ...
            job->next_run = cron_next_daily_epoch(job->hour, job->minute, now);
        }
...
esp_err_t cron_add_job(cron_job_t *job)
{
    ...
    ESP_LOGI(TAG, "Added cron job: %s (%s) kind=%s next_run=%lld",
             ...
             (job->kind == CRON_KIND_DAILY ? "daily" : "at"),
